...
export default class Sync extends Emitter {
        ...
        const disposable = atom.project.onDidChangeFiles(events => {
            for (const event of events) {
                if (this.config.useSync()) {
                    ...
                    if (!projectConfig || !projectConfig.sync || this.ignorePath(projectConfig, event.path, projectConfig.path)) {
                        return;
                    }
                    switch (event.action) {
                        case "modified":
    ...
    store(projectConfig, file, add) {
        ...
        const url = connection.server + "/apps/atom-editor/store" + projectConfig.sync.root + "/" + relPath;
        ...
        options = {
            ...
            method: "PUT",
            auth: {
                user: projectConfig.sync.user || connection.user,
                pass: projectConfig.sync.password || connection.password || "",
    ...
    remove(projectConfig, file) {
        const relPath = atom.project.relativizePath(file)[1];
        const connection = projectConfig.servers[projectConfig.sync.server];
        const url = connection.server + "/apps/atom-editor/delete" + projectConfig.sync.root + "/" + relPath;
        self = this;
        options = {
            uri: url,
            method: "GET",
    ...
    query(projectConfig, query, onError, onSuccess) {
        const connection = projectConfig.servers[projectConfig.sync.server];
        const url = connection.server + "/apps/atom-editor/run?q=" + encodeURIComponent(query);
        options = {
            uri: url,
            method: "GET",
            json: true,
